...
namespace vkBasalt
{
        ...
        const DepthState& depth)
    {
        ...
        writeCommandBuffers(pLogicalDevice, pLogicalSwapchain->effects,
                           ...
                           static_cast<uint32_t>(
                               pLogicalSwapchain->timingEffectNames.size()));
        ...
        const std::vector<std::string>& effectStrings)
    {
        ...
        for (uint32_t i = 0; i < effectStrings.size(); i++)
        {
            ...
            if (effectRegistry.hasEffectFailed(effectStrings[i]))
            {
    ...
    void updateOverlayState(LogicalDevice* pLogicalDevice, bool effectsEnabled)
    {
        ...
        for (const auto& [_, swapchain] : swapchainMap)
        {
            if (!swapchain || swapchain->pLogicalDevice != pLogicalDevice
                || swapchain->timingQueryPool == VK_NULL_HANDLE)
                continue;
        ...
        if (timingSwapchain != nullptr)
        {
            ...
            for (size_t i = 0; i < timingSwapchain->timingEffectNames.size(); ++i)
            {
                if (i >= timingSwapchain->timingMilliseconds.size()
                    ...
                    continue;
                overlayState.effectGpuMilliseconds[
                    timingSwapchain->timingEffectNames[i]] =
                    timingSwapchain->timingMilliseconds[i];
                overlayState.totalEffectGpuMilliseconds +=
                    timingSwapchain->timingMilliseconds[i];
            }
        }
        ...
        for (uint32_t i = 0; i < pCreateInfo->queueCreateInfoCount; i++)
        {
            ...
            if ((queueProperties[queueInfo.queueFamilyIndex].queueFlags & VK_QUEUE_GRAPHICS_BIT))
            {
                ...
                pLogicalDevice->timestampPeriodNanoseconds =
                    deviceProps.limits.timestampPeriod;
                pLogicalDevice->timestampValidBits =
                    queueProperties[queueInfo.queueFamilyIndex].timestampValidBits;
    ...
    void warmEffectCompileCache(VkSwapchainKHR swapchain)
    {
...

        {
            ...
            requests = gatherEffectCompileRequests(effectRegistry.getActiveEffects(),
                                                   pLogicalSwapchain->imageExtent,
                                                                  ...
                                                                  VkImage*       pSwapchainImages)
    {
        ...
        const std::vector<std::string> activeEffects = effectRegistry.getActiveEffects();
    ...
    VKAPI_ATTR VkResult VKAPI_CALL vkBasalt_QueuePresentKHR(VkQueue queue, const VkPresentInfoKHR* pPresentInfo)
    {
        ...
        if (shouldReload)
        {
            ...
            else
            {
                ...
                std::vector<std::string> activeEffects = pLogicalDevice->imguiOverlay
                    ...
                    : effectRegistry.getActiveEffects();
        ...
        for (unsigned int i = 0; i < pPresentInfo->swapchainCount; i++)
        {
            ...
            if (effectFence != VK_NULL_HANDLE)
            {
                ...
                const VkResult timingResult = pLogicalSwapchain->collectEffectTimings(index);
                if (timingResult != VK_SUCCESS)
                {
                    Logger::warn("per-effect timestamp read failed for image "
                                 + std::to_string(index) + ": "
                                 + std::to_string(timingResult));
                }
