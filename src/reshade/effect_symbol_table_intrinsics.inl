...
IMPLEMENT_INTRINSIC_SPIRV(tex1D, 1, {
	...
	const spv::Id sample_coords = args[2].is_constant
		? args[1].base
		: fold_offset_into_coords(args[0], args[1].base, args[2].base, res_type.rows == 1 ? 1 : 2);

	spirv_instruction &sample_inst = add_instruction(spv::OpImageSampleImplicitLod, convert_type(res_vector_type))
		.add(args[0].base)
		.add(sample_coords)
		.add(args[2].is_constant ? spv::ImageOperandsConstOffsetMask : 0)
		;
	if (args[2].is_constant)
		sample_inst.add(args[2].base);
	const spv::Id res = sample_inst.result;
	if (res_type.rows == 1)
		return
		add_instruction(spv::OpCompositeExtract, convert_type(res_type))
			.add(res)
...
IMPLEMENT_INTRINSIC_SPIRV(tex1Dgrad, 1, {
	...
	spirv_instruction &sample_inst = add_instruction(spv::OpImageSampleExplicitLod, convert_type(res_vector_type))
		.add(args[0].base)
		.add(args[1].base)
		.add(spv::ImageOperandsGradMask | (args[4].is_constant ? spv::ImageOperandsConstOffsetMask : 0))
		.add(args[2].base)
		.add(args[3].base)
		;
	if (args[4].is_constant)
		sample_inst.add(args[4].base);
	const spv::Id res = sample_inst.result;
	if (res_type.rows == 1)
		return
		add_instruction(spv::OpCompositeExtract, convert_type(res_type))
...
IMPLEMENT_INTRINSIC_SPIRV(tex1Dlod, 1, {
	...
	spirv_instruction &sample_inst = add_instruction(spv::OpImageSampleExplicitLod, convert_type(res_vector_type))
		.add(args[0].base)
		.add(x)
		.add(spv::ImageOperandsLodMask | (args[2].is_constant ? spv::ImageOperandsConstOffsetMask : 0))
		.add(lod)
		;
	if (args[2].is_constant)
		sample_inst.add(args[2].base);
