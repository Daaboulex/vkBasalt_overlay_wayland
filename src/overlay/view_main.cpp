    ...
    void ImGuiOverlay::renderMainView(const KeyboardState& keyboard)
    {
        ...
        if (state.effectGpuTimingSupported)
        {
            ...
            if (state.effectGpuMilliseconds.empty())
                ...
                ImGui::TextDisabled("GPU: %.2f ms last", state.totalEffectGpuMilliseconds);
            else
                ImGui::TextDisabled("GPU: %.2f ms total", state.totalEffectGpuMilliseconds);
            if (ImGui::IsItemHovered())
        ...
        for (size_t i = 0; i < selectedEffects.size(); i++)
        {
            ...
            {
                char timingLabel[32];
                std::snprintf(timingLabel, sizeof(timingLabel), "%.2f ms", timing->second);
